...
define(['utils', 'cell/View', 'cell/Ext'], function(utils, View, Ext) {
  var AttrBind, Bind, BindNeedRender, EachBind, ExtBind, HashQueue, IfBind, hashkey, hashuid, isBind, nextuid, orig__each, orig__if, orig_constructor, orig_remove, orig_renderAttr, orig_renderChild, __;
  isBind = function(o) {
    return typeof o === 'function';
  };
  Ext.prototype.getValue = function(v, callback) {
    if (isBind(v)) {
      this.view._binds.push(new ExtBind(callback, v));
    } else {
      callback(v);
    }
  };
  ...
  ExtBind = function(cb, getValue) {
    this.cb = cb;
    this.getValue = getValue;
    this.cb(this.value = this.getValue());
  };
  ExtBind.prototype = {
  ...
  View.prototype._renderAttr = function(k, v, parent) {
    var binding;
    if (isBind(v)) {
      this._binds.push(binding = new AttrBind(parent, k, utils.bind(v, this)));
      binding.needRender();
      binding.render(this);
    } else {
  ...
  View.prototype._renderChild = function(n, parent, insertBeforeNode, rendered) {
    if (isBind(n)) {
      n = new Bind(parent, utils.bind(n, this));
    }

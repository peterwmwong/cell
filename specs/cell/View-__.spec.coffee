define ['../utils/spec-utils'], ({nodeHTMLEquals,stringify,node,browserTrigger})->
  ...
  ({beforeEachRequire})->
    ...
    for useViewBindings in [true, false] then do(useViewBindings)->

      describe "#{if useViewBindings then 'WITH' else 'WITHOUT'} cell/opts/ViewBindings", ->
        ...
        beforeEachRun = (@TestCell1,@View)->
          @view = new @View()
          @__ = @view.__

        if useViewBindings
          beforeEachRequire ['cell/opts/ViewBindings'], DEPS, beforeEachRun
        else
          beforeEachRequire DEPS, beforeEachRun

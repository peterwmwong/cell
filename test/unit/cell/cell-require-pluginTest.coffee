define ['/test/util/reset-requirejs-plugins.js'], (resetplugins) ->
   ...
   'require(cell!, cell!) repeated calls' : (require, get, done) ->
      ...
      get (crp) ->
         ...
         ok not cellLoadedSpy1.called, "Cell loaded callback shouldn't be called until it's actually loaded"
         ...
         defer ->
            ...
            equal CellSpy.callCount, 2, "Cell shouldn't be called after second require call"
   ...
   'require(cell!, cell!) repeated calls before load' : (require, get, done) ->
